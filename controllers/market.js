...
router.post('/:marketId/comments', async (req, res) => {
    try {
      req.body.author = req.user._id;
      const market = await marketPlace.findById(req.params.marketId);
      market.comments.push(req.body);
      await market.save();
  
      // Find the newly created comment:
      const newComment = market.comments[market.comments.length - 1];
  ...
  router.put('/:marketId/comments/:commentId', async (req, res) => {
    try {
      const market = await marketPlace.findById(req.params.marketId);
      const comment = market.comments.id(req.params.commentId);
  ...
  router.delete('/:marketId/comments/:commentId', async (req, res) => {
    try {
      const market = await marketPlace.findById(req.params.marketId);
      market.comments.remove({ _id: req.params.commentId });
      await market.save();
      res.status(200).json({ message: 'Ok' });
    } catch (err) {
      res.status(500).json(err);
